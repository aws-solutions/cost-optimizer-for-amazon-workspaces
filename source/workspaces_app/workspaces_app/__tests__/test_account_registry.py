...
def test_ddb_registry():
    ...
    mock_table.scan.assert_called_once_with(
        ProjectionExpression="account_id, role_name"
    )
...
def test_ddb_registry_exception():
    ...
    mock_table.scan.side_effect = ClientError(
        {"Error": {"Code": "InternalServerError", "Message": "message"}}, "scan"
    )
...
@patch.dict(
    os.environ,
    {"UserAgentString": "user-agent", "SpokeAccountDynamoDBTable": "ddb-table"},
)
...
def test_get_account_registry(mock_session, mock_ddb_registry):
    ...
    mock_session.return_value.resource.side_effect = lambda resource, **_: (
        mock_ddb if resource == "dynamodb" else Exception()
    )
