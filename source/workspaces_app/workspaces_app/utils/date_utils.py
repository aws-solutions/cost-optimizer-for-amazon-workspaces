...
def get_first_day_selected_month():
    relative_date = datetime.datetime.utcnow() - relativedelta(months=int(NUMBER_OF_MONTHS) - 1)
    first_date_selected_month = relative_date.replace(day=1, hour=0, minute=0, second=0, microsecond=0).date()
    log.debug(f'First day of selected month is {first_date_selected_month}')
...
def get_start_end_time_first_day_selected_month():
    ...
    star_time_selected_date = first_date_selected_month.strftime(YEAR_MONTH_STRING) + '-01T00:00:00Z'
    end_time_selected_date = first_date_selected_month.strftime(YEAR_MONTH_STRING) + '-02T00:00:00Z'
    log.debug(f'Start time for first day of selected month is {star_time_selected_date}')
    log.debug(f'End time for first day of selected month is {end_time_selected_date}')
...
def is_last_day_current_month():
    ...
    log.debug(f'Today is {today}')
    log.debug(f'Last day of month is {last_day}')
    return today == last_day


def get_date_for_today():
    date_for_today = (time.strftime('%D', time.gmtime()))
    log.info(f'Returning today date as {date_for_today}')
...
def get_date_time_values_for_processing():
    ...
    star_time_selected_date, end_time_selected_date = get_start_end_time_first_day_selected_month()
...
def get_report_time_for_s3_key():
    ...
    log.debug(f"Returning time for s3 key as {time_for_s3_key}")
