...
class DynamoDBTable:
    ...
    def put_item(self, account_id: str, role_name: str) -> None:
        ...
        Raises:
            ClientError:
        """
        log.debug(f"Calling put_item with {account_id[-4:]} and {role_name.split('/')[1]}")
        try:
            self.table.put_item(
                Item={
            ...
            log.error(f"Error when adding the account id {account_id[-4:]} to dynamodb: {e}")
            raise e
    ...
    def delete_item(self, account_id: str, role_name: str) -> None:
        ...
        Raises:
            ClientError:
        """
        log.debug(f"Calling delete_item with {account_id[-4:]} and {role_name.split('/')[1]}")
        try:
            ...
            log.error(f"Error when deleting the account id {account_id[-4:]} from dynamodb: {e}")
            raise e
