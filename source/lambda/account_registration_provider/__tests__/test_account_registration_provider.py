...
def mock_env():
    patch = unittest.mock.patch.dict(
        os.environ,
        {"REGISTER_LAMBDA_ARN": "some-arn", "MANAGEMENT_ROLE_ARN": "some-other-arn"},
    )
    patch.start()
    yield
    patch.stop()
...
@pytest.fixture()
def mock_clients():
    clients = {"sts": boto3.client("sts"), "lambda": boto3.client("lambda")}
...
def stub_clients(mock_clients):
    stubs = {
        service: botocore.stub.Stubber(client)
...
def successful_payload():
    return {"status": {"code": "Success", "message": "reason"}}


@unittest.mock.patch("cfnresponse.send")
def test_event_handler_create(
    mock_cfnresponse_send, stub_clients, stub_get_caller_identity
):
    ...
    stub_clients["lambda"].add_response(
        "invoke",
        response,
        {
...
@unittest.mock.patch("cfnresponse.send")
def test_event_handler_fatal_status(
    mock_cfnresponse_send, stub_clients, stub_get_caller_identity
):
    event = {"RequestType": "Create"}
    context = MockContext()
    ...
    account_registration_provider.event_handler(event, context)
    mock_cfnresponse_send.assert_called_once_with(
        event, context, cfnresponse.FAILED, response, reason=unittest.mock.ANY
    )


@unittest.mock.patch("cfnresponse.send")
def test_event_handler_exception(
    mock_cfnresponse_send, stub_clients, stub_get_caller_identity
):
    event = {"RequestType": "Delete"}
    context = MockContext()
    stub_clients["lambda"].add_client_error(
        "invoke",
        "ServiceException",
        expected_params={
            "FunctionName": "some-arn",
    ...
    account_registration_provider.event_handler(event, context)
    mock_cfnresponse_send.assert_called_once_with(
        event, context, cfnresponse.FAILED, {}, reason=unittest.mock.ANY
    )


@unittest.mock.patch("cfnresponse.send")
def test_event_handler_registration_failed(
    mock_cfnresponse_send, stub_clients, stub_get_caller_identity
):
    event = {"RequestType": "Delete"}
    context = MockContext()
    response = {
        ...
        "Payload": response_body(
            {"status": {"code": "Failed", "message": "reason", "error": "error"}}
        ),
        "ExecutedVersion": "string",
    }
    stub_clients["lambda"].add_response(
...
@unittest.mock.patch("cfnresponse.send")
def test_event_handler_response_serializable(
    mock_cfnresponse_send, stub_clients, stub_get_caller_identity
):
    event = {"RequestType": "Create"}
    context = MockContext()
    ...
    account_registration_provider.event_handler(event, context)
    mock_cfnresponse_send.assert_called_once_with(
        event, context, cfnresponse.SUCCESS, unittest.mock.ANY
    )
